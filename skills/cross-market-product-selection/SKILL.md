...
5. 平台产品总评分固定为：销量标准分 × 40% + 价格相似分 × 40% + 评价标准分 × 20%；固定权重不可改写，也不得把该分数冒充联合最终配对分。
...
严格合格且销量、实际价格、评价星级证据完整且无冲突的候选，才按固定平台公式评分；保留原始值、标准分、固定权重、加权分和平台产品总分。销量只与同模式、同来源、同周期的严格行归一化，Amazon 还必须同站点。销量、实际价格或评价星级任一缺失或冲突时，转为待核验，不计算总评分并从严格结果移出。两端联动时，市场机会、供货能力和匹配质量分开呈现，不用高需求掩盖供应风险；任务说明未确认最终配对公式和三类权重时，最终配对得分与排名留空。导出前核验主图、链接、评分公式、状态分表、筛选和图片行对应关系。
