...
严格合格且销量、实际价格、评价星级证据完整且无冲突的候选，才按已确认的公式评分；保留原始值、标准分、权重、加权分和总分。销量、实际价格或评价星级任一缺失或冲突时，转为待核验，不计算总评分并从严格结果移出。两端联动时，市场机会、供货能力和匹配质量分开呈现，不用高需求掩盖供应风险。导出前核验主图、链接、评分公式、状态分表、筛选和图片行对应关系。
